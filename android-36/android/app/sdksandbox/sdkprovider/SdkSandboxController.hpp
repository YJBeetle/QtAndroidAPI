#pragma once

#include "../../../os/Bundle.def.hpp"
#include "../../../../JString.hpp"
#include "./SdkSandboxController.def.hpp"

namespace android::app::sdksandbox::sdkprovider
{
	// Fields
	inline JString SdkSandboxController::SDK_SANDBOX_CONTROLLER_SERVICE()
	{
		return getStaticObjectField(
			"android.app.sdksandbox.sdkprovider.SdkSandboxController",
			"SDK_SANDBOX_CONTROLLER_SERVICE",
			"Ljava/lang/String;"
		);
	}
	
	// Constructors
	
	// Methods
	inline JObject SdkSandboxController::getAppOwnedSdkSandboxInterfaces() const
	{
		return callObjectMethod(
			"getAppOwnedSdkSandboxInterfaces",
			"()Ljava/util/List;"
		);
	}
	inline JString SdkSandboxController::getClientPackageName() const
	{
		return callObjectMethod(
			"getClientPackageName",
			"()Ljava/lang/String;"
		);
	}
	inline JObject SdkSandboxController::getClientSharedPreferences() const
	{
		return callObjectMethod(
			"getClientSharedPreferences",
			"()Landroid/content/SharedPreferences;"
		);
	}
	inline JObject SdkSandboxController::getSandboxedSdks() const
	{
		return callObjectMethod(
			"getSandboxedSdks",
			"()Ljava/util/List;"
		);
	}
	inline void SdkSandboxController::loadSdk(JString arg0, android::os::Bundle arg1, JObject arg2, JObject arg3) const
	{
		callMethod<void>(
			"loadSdk",
			"(Ljava/lang/String;Landroid/os/Bundle;Ljava/util/concurrent/Executor;Landroid/os/OutcomeReceiver;)V",
			arg0.object<jstring>(),
			arg1.object(),
			arg2.object(),
			arg3.object()
		);
	}
	inline JObject SdkSandboxController::registerSdkSandboxActivityHandler(JObject arg0) const
	{
		return callObjectMethod(
			"registerSdkSandboxActivityHandler",
			"(Landroid/app/sdksandbox/sdkprovider/SdkSandboxActivityHandler;)Landroid/os/IBinder;",
			arg0.object()
		);
	}
	inline void SdkSandboxController::registerSdkSandboxClientImportanceListener(JObject arg0, JObject arg1) const
	{
		callMethod<void>(
			"registerSdkSandboxClientImportanceListener",
			"(Ljava/util/concurrent/Executor;Landroid/app/sdksandbox/sdkprovider/SdkSandboxClientImportanceListener;)V",
			arg0.object(),
			arg1.object()
		);
	}
	inline void SdkSandboxController::unregisterSdkSandboxActivityHandler(JObject arg0) const
	{
		callMethod<void>(
			"unregisterSdkSandboxActivityHandler",
			"(Landroid/app/sdksandbox/sdkprovider/SdkSandboxActivityHandler;)V",
			arg0.object()
		);
	}
	inline void SdkSandboxController::unregisterSdkSandboxClientImportanceListener(JObject arg0) const
	{
		callMethod<void>(
			"unregisterSdkSandboxClientImportanceListener",
			"(Landroid/app/sdksandbox/sdkprovider/SdkSandboxClientImportanceListener;)V",
			arg0.object()
		);
	}
} // namespace android::app::sdksandbox::sdkprovider

// Base class headers

#ifdef QT_ANDROID_API_AUTOUSE
using namespace android::app::sdksandbox::sdkprovider;
#endif
