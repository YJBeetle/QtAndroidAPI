#pragma once

#include "./GetByDocumentIdRequest.def.hpp"
#include "./ReportSystemUsageRequest.def.hpp"
#include "./SearchResults.def.hpp"
#include "./SearchSpec.def.hpp"
#include "./observer/ObserverSpec.def.hpp"
#include "../../../JString.hpp"
#include "./GlobalSearchSession.def.hpp"

namespace android::app::appsearch
{
	// Fields
	
	// Constructors
	
	// Methods
	inline void GlobalSearchSession::close() const
	{
		callMethod<void>(
			"close",
			"()V"
		);
	}
	inline void GlobalSearchSession::getByDocumentId(JString arg0, JString arg1, android::app::appsearch::GetByDocumentIdRequest arg2, JObject arg3, JObject arg4) const
	{
		callMethod<void>(
			"getByDocumentId",
			"(Ljava/lang/String;Ljava/lang/String;Landroid/app/appsearch/GetByDocumentIdRequest;Ljava/util/concurrent/Executor;Landroid/app/appsearch/BatchResultCallback;)V",
			arg0.object<jstring>(),
			arg1.object<jstring>(),
			arg2.object(),
			arg3.object(),
			arg4.object()
		);
	}
	inline void GlobalSearchSession::getSchema(JString arg0, JString arg1, JObject arg2, JObject arg3) const
	{
		callMethod<void>(
			"getSchema",
			"(Ljava/lang/String;Ljava/lang/String;Ljava/util/concurrent/Executor;Ljava/util/function/Consumer;)V",
			arg0.object<jstring>(),
			arg1.object<jstring>(),
			arg2.object(),
			arg3.object()
		);
	}
	inline void GlobalSearchSession::openBlobForRead(JObject arg0, JObject arg1, JObject arg2) const
	{
		callMethod<void>(
			"openBlobForRead",
			"(Ljava/util/Set;Ljava/util/concurrent/Executor;Ljava/util/function/Consumer;)V",
			arg0.object(),
			arg1.object(),
			arg2.object()
		);
	}
	inline void GlobalSearchSession::registerObserverCallback(JString arg0, android::app::appsearch::observer::ObserverSpec arg1, JObject arg2, JObject arg3) const
	{
		callMethod<void>(
			"registerObserverCallback",
			"(Ljava/lang/String;Landroid/app/appsearch/observer/ObserverSpec;Ljava/util/concurrent/Executor;Landroid/app/appsearch/observer/ObserverCallback;)V",
			arg0.object<jstring>(),
			arg1.object(),
			arg2.object(),
			arg3.object()
		);
	}
	inline void GlobalSearchSession::reportSystemUsage(android::app::appsearch::ReportSystemUsageRequest arg0, JObject arg1, JObject arg2) const
	{
		callMethod<void>(
			"reportSystemUsage",
			"(Landroid/app/appsearch/ReportSystemUsageRequest;Ljava/util/concurrent/Executor;Ljava/util/function/Consumer;)V",
			arg0.object(),
			arg1.object(),
			arg2.object()
		);
	}
	inline android::app::appsearch::SearchResults GlobalSearchSession::search(JString arg0, android::app::appsearch::SearchSpec arg1) const
	{
		return callObjectMethod(
			"search",
			"(Ljava/lang/String;Landroid/app/appsearch/SearchSpec;)Landroid/app/appsearch/SearchResults;",
			arg0.object<jstring>(),
			arg1.object()
		);
	}
	inline void GlobalSearchSession::unregisterObserverCallback(JString arg0, JObject arg1) const
	{
		callMethod<void>(
			"unregisterObserverCallback",
			"(Ljava/lang/String;Landroid/app/appsearch/observer/ObserverCallback;)V",
			arg0.object<jstring>(),
			arg1.object()
		);
	}
} // namespace android::app::appsearch

// Base class headers

#ifdef QT_ANDROID_API_AUTOUSE
using namespace android::app::appsearch;
#endif
