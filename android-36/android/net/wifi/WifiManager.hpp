#pragma once

#include "../DhcpInfo.def.hpp"
#include "./BlockingOption.def.hpp"
#include "./SoftApConfiguration.def.hpp"
#include "./WifiConfiguration.def.hpp"
#include "./WifiInfo.def.hpp"
#include "./WifiManager_AddNetworkResult.def.hpp"
#include "./WifiManager_LocalOnlyHotspotCallback.def.hpp"
#include "./WifiManager_MulticastLock.def.hpp"
#include "./WifiManager_ScanResultsCallback.def.hpp"
#include "./WifiManager_SubsystemRestartTrackingCallback.def.hpp"
#include "./WifiManager_WifiLock.def.hpp"
#include "./WifiManager_WpsCallback.def.hpp"
#include "./WifiSsid.def.hpp"
#include "./WpsInfo.def.hpp"
#include "./hotspot2/PasspointConfiguration.def.hpp"
#include "../../os/Handler.def.hpp"
#include "../../../JString.hpp"
#include "../../../java/net/InetAddress.def.hpp"
#include "./WifiManager.def.hpp"

namespace android::net::wifi
{
	// Fields
	inline JString WifiManager::ACTION_PICK_WIFI_NETWORK()
	{
		return getStaticObjectField(
			"android.net.wifi.WifiManager",
			"ACTION_PICK_WIFI_NETWORK",
			"Ljava/lang/String;"
		);
	}
	inline jint WifiManager::ACTION_REMOVE_SUGGESTION_DISCONNECT()
	{
		return getStaticField<jint>(
			"android.net.wifi.WifiManager",
			"ACTION_REMOVE_SUGGESTION_DISCONNECT"
		);
	}
	inline jint WifiManager::ACTION_REMOVE_SUGGESTION_LINGER()
	{
		return getStaticField<jint>(
			"android.net.wifi.WifiManager",
			"ACTION_REMOVE_SUGGESTION_LINGER"
		);
	}
	inline JString WifiManager::ACTION_REQUEST_SCAN_ALWAYS_AVAILABLE()
	{
		return getStaticObjectField(
			"android.net.wifi.WifiManager",
			"ACTION_REQUEST_SCAN_ALWAYS_AVAILABLE",
			"Ljava/lang/String;"
		);
	}
	inline JString WifiManager::ACTION_WIFI_NETWORK_SUGGESTION_POST_CONNECTION()
	{
		return getStaticObjectField(
			"android.net.wifi.WifiManager",
			"ACTION_WIFI_NETWORK_SUGGESTION_POST_CONNECTION",
			"Ljava/lang/String;"
		);
	}
	inline JString WifiManager::ACTION_WIFI_SCAN_AVAILABILITY_CHANGED()
	{
		return getStaticObjectField(
			"android.net.wifi.WifiManager",
			"ACTION_WIFI_SCAN_AVAILABILITY_CHANGED",
			"Ljava/lang/String;"
		);
	}
	inline JString WifiManager::CHANNEL_DATA_KEY_FREQUENCY_MHZ()
	{
		return getStaticObjectField(
			"android.net.wifi.WifiManager",
			"CHANNEL_DATA_KEY_FREQUENCY_MHZ",
			"Ljava/lang/String;"
		);
	}
	inline JString WifiManager::CHANNEL_DATA_KEY_NUM_AP()
	{
		return getStaticObjectField(
			"android.net.wifi.WifiManager",
			"CHANNEL_DATA_KEY_NUM_AP",
			"Ljava/lang/String;"
		);
	}
	inline jint WifiManager::ERROR_AUTHENTICATING()
	{
		return getStaticField<jint>(
			"android.net.wifi.WifiManager",
			"ERROR_AUTHENTICATING"
		);
	}
	inline JString WifiManager::EXTRA_BSSID()
	{
		return getStaticObjectField(
			"android.net.wifi.WifiManager",
			"EXTRA_BSSID",
			"Ljava/lang/String;"
		);
	}
	inline JString WifiManager::EXTRA_NETWORK_INFO()
	{
		return getStaticObjectField(
			"android.net.wifi.WifiManager",
			"EXTRA_NETWORK_INFO",
			"Ljava/lang/String;"
		);
	}
	inline JString WifiManager::EXTRA_NETWORK_SUGGESTION()
	{
		return getStaticObjectField(
			"android.net.wifi.WifiManager",
			"EXTRA_NETWORK_SUGGESTION",
			"Ljava/lang/String;"
		);
	}
	inline JString WifiManager::EXTRA_NEW_RSSI()
	{
		return getStaticObjectField(
			"android.net.wifi.WifiManager",
			"EXTRA_NEW_RSSI",
			"Ljava/lang/String;"
		);
	}
	inline JString WifiManager::EXTRA_NEW_STATE()
	{
		return getStaticObjectField(
			"android.net.wifi.WifiManager",
			"EXTRA_NEW_STATE",
			"Ljava/lang/String;"
		);
	}
	inline JString WifiManager::EXTRA_PREVIOUS_WIFI_STATE()
	{
		return getStaticObjectField(
			"android.net.wifi.WifiManager",
			"EXTRA_PREVIOUS_WIFI_STATE",
			"Ljava/lang/String;"
		);
	}
	inline JString WifiManager::EXTRA_RESULTS_UPDATED()
	{
		return getStaticObjectField(
			"android.net.wifi.WifiManager",
			"EXTRA_RESULTS_UPDATED",
			"Ljava/lang/String;"
		);
	}
	inline JString WifiManager::EXTRA_SCAN_AVAILABLE()
	{
		return getStaticObjectField(
			"android.net.wifi.WifiManager",
			"EXTRA_SCAN_AVAILABLE",
			"Ljava/lang/String;"
		);
	}
	inline JString WifiManager::EXTRA_SUPPLICANT_CONNECTED()
	{
		return getStaticObjectField(
			"android.net.wifi.WifiManager",
			"EXTRA_SUPPLICANT_CONNECTED",
			"Ljava/lang/String;"
		);
	}
	inline JString WifiManager::EXTRA_SUPPLICANT_ERROR()
	{
		return getStaticObjectField(
			"android.net.wifi.WifiManager",
			"EXTRA_SUPPLICANT_ERROR",
			"Ljava/lang/String;"
		);
	}
	inline JString WifiManager::EXTRA_WIFI_INFO()
	{
		return getStaticObjectField(
			"android.net.wifi.WifiManager",
			"EXTRA_WIFI_INFO",
			"Ljava/lang/String;"
		);
	}
	inline JString WifiManager::EXTRA_WIFI_STATE()
	{
		return getStaticObjectField(
			"android.net.wifi.WifiManager",
			"EXTRA_WIFI_STATE",
			"Ljava/lang/String;"
		);
	}
	inline jint WifiManager::FLAG_SEND_DHCP_HOSTNAME_RESTRICTION_OPEN()
	{
		return getStaticField<jint>(
			"android.net.wifi.WifiManager",
			"FLAG_SEND_DHCP_HOSTNAME_RESTRICTION_OPEN"
		);
	}
	inline jint WifiManager::FLAG_SEND_DHCP_HOSTNAME_RESTRICTION_SECURE()
	{
		return getStaticField<jint>(
			"android.net.wifi.WifiManager",
			"FLAG_SEND_DHCP_HOSTNAME_RESTRICTION_SECURE"
		);
	}
	inline JString WifiManager::NETWORK_IDS_CHANGED_ACTION()
	{
		return getStaticObjectField(
			"android.net.wifi.WifiManager",
			"NETWORK_IDS_CHANGED_ACTION",
			"Ljava/lang/String;"
		);
	}
	inline JString WifiManager::NETWORK_STATE_CHANGED_ACTION()
	{
		return getStaticObjectField(
			"android.net.wifi.WifiManager",
			"NETWORK_STATE_CHANGED_ACTION",
			"Ljava/lang/String;"
		);
	}
	inline jint WifiManager::ROAMING_MODE_AGGRESSIVE()
	{
		return getStaticField<jint>(
			"android.net.wifi.WifiManager",
			"ROAMING_MODE_AGGRESSIVE"
		);
	}
	inline jint WifiManager::ROAMING_MODE_NONE()
	{
		return getStaticField<jint>(
			"android.net.wifi.WifiManager",
			"ROAMING_MODE_NONE"
		);
	}
	inline jint WifiManager::ROAMING_MODE_NORMAL()
	{
		return getStaticField<jint>(
			"android.net.wifi.WifiManager",
			"ROAMING_MODE_NORMAL"
		);
	}
	inline JString WifiManager::RSSI_CHANGED_ACTION()
	{
		return getStaticObjectField(
			"android.net.wifi.WifiManager",
			"RSSI_CHANGED_ACTION",
			"Ljava/lang/String;"
		);
	}
	inline JString WifiManager::SCAN_RESULTS_AVAILABLE_ACTION()
	{
		return getStaticObjectField(
			"android.net.wifi.WifiManager",
			"SCAN_RESULTS_AVAILABLE_ACTION",
			"Ljava/lang/String;"
		);
	}
	inline jint WifiManager::STATUS_LOCAL_ONLY_CONNECTION_FAILURE_ASSOCIATION()
	{
		return getStaticField<jint>(
			"android.net.wifi.WifiManager",
			"STATUS_LOCAL_ONLY_CONNECTION_FAILURE_ASSOCIATION"
		);
	}
	inline jint WifiManager::STATUS_LOCAL_ONLY_CONNECTION_FAILURE_AUTHENTICATION()
	{
		return getStaticField<jint>(
			"android.net.wifi.WifiManager",
			"STATUS_LOCAL_ONLY_CONNECTION_FAILURE_AUTHENTICATION"
		);
	}
	inline jint WifiManager::STATUS_LOCAL_ONLY_CONNECTION_FAILURE_IP_PROVISIONING()
	{
		return getStaticField<jint>(
			"android.net.wifi.WifiManager",
			"STATUS_LOCAL_ONLY_CONNECTION_FAILURE_IP_PROVISIONING"
		);
	}
	inline jint WifiManager::STATUS_LOCAL_ONLY_CONNECTION_FAILURE_NOT_FOUND()
	{
		return getStaticField<jint>(
			"android.net.wifi.WifiManager",
			"STATUS_LOCAL_ONLY_CONNECTION_FAILURE_NOT_FOUND"
		);
	}
	inline jint WifiManager::STATUS_LOCAL_ONLY_CONNECTION_FAILURE_NO_RESPONSE()
	{
		return getStaticField<jint>(
			"android.net.wifi.WifiManager",
			"STATUS_LOCAL_ONLY_CONNECTION_FAILURE_NO_RESPONSE"
		);
	}
	inline jint WifiManager::STATUS_LOCAL_ONLY_CONNECTION_FAILURE_UNKNOWN()
	{
		return getStaticField<jint>(
			"android.net.wifi.WifiManager",
			"STATUS_LOCAL_ONLY_CONNECTION_FAILURE_UNKNOWN"
		);
	}
	inline jint WifiManager::STATUS_LOCAL_ONLY_CONNECTION_FAILURE_USER_REJECT()
	{
		return getStaticField<jint>(
			"android.net.wifi.WifiManager",
			"STATUS_LOCAL_ONLY_CONNECTION_FAILURE_USER_REJECT"
		);
	}
	inline jint WifiManager::STATUS_NETWORK_SUGGESTIONS_ERROR_ADD_DUPLICATE()
	{
		return getStaticField<jint>(
			"android.net.wifi.WifiManager",
			"STATUS_NETWORK_SUGGESTIONS_ERROR_ADD_DUPLICATE"
		);
	}
	inline jint WifiManager::STATUS_NETWORK_SUGGESTIONS_ERROR_ADD_EXCEEDS_MAX_PER_APP()
	{
		return getStaticField<jint>(
			"android.net.wifi.WifiManager",
			"STATUS_NETWORK_SUGGESTIONS_ERROR_ADD_EXCEEDS_MAX_PER_APP"
		);
	}
	inline jint WifiManager::STATUS_NETWORK_SUGGESTIONS_ERROR_ADD_INVALID()
	{
		return getStaticField<jint>(
			"android.net.wifi.WifiManager",
			"STATUS_NETWORK_SUGGESTIONS_ERROR_ADD_INVALID"
		);
	}
	inline jint WifiManager::STATUS_NETWORK_SUGGESTIONS_ERROR_ADD_NOT_ALLOWED()
	{
		return getStaticField<jint>(
			"android.net.wifi.WifiManager",
			"STATUS_NETWORK_SUGGESTIONS_ERROR_ADD_NOT_ALLOWED"
		);
	}
	inline jint WifiManager::STATUS_NETWORK_SUGGESTIONS_ERROR_APP_DISALLOWED()
	{
		return getStaticField<jint>(
			"android.net.wifi.WifiManager",
			"STATUS_NETWORK_SUGGESTIONS_ERROR_APP_DISALLOWED"
		);
	}
	inline jint WifiManager::STATUS_NETWORK_SUGGESTIONS_ERROR_INTERNAL()
	{
		return getStaticField<jint>(
			"android.net.wifi.WifiManager",
			"STATUS_NETWORK_SUGGESTIONS_ERROR_INTERNAL"
		);
	}
	inline jint WifiManager::STATUS_NETWORK_SUGGESTIONS_ERROR_REMOVE_INVALID()
	{
		return getStaticField<jint>(
			"android.net.wifi.WifiManager",
			"STATUS_NETWORK_SUGGESTIONS_ERROR_REMOVE_INVALID"
		);
	}
	inline jint WifiManager::STATUS_NETWORK_SUGGESTIONS_ERROR_RESTRICTED_BY_ADMIN()
	{
		return getStaticField<jint>(
			"android.net.wifi.WifiManager",
			"STATUS_NETWORK_SUGGESTIONS_ERROR_RESTRICTED_BY_ADMIN"
		);
	}
	inline jint WifiManager::STATUS_NETWORK_SUGGESTIONS_SUCCESS()
	{
		return getStaticField<jint>(
			"android.net.wifi.WifiManager",
			"STATUS_NETWORK_SUGGESTIONS_SUCCESS"
		);
	}
	inline jint WifiManager::STATUS_SUGGESTION_APPROVAL_APPROVED_BY_CARRIER_PRIVILEGE()
	{
		return getStaticField<jint>(
			"android.net.wifi.WifiManager",
			"STATUS_SUGGESTION_APPROVAL_APPROVED_BY_CARRIER_PRIVILEGE"
		);
	}
	inline jint WifiManager::STATUS_SUGGESTION_APPROVAL_APPROVED_BY_USER()
	{
		return getStaticField<jint>(
			"android.net.wifi.WifiManager",
			"STATUS_SUGGESTION_APPROVAL_APPROVED_BY_USER"
		);
	}
	inline jint WifiManager::STATUS_SUGGESTION_APPROVAL_PENDING()
	{
		return getStaticField<jint>(
			"android.net.wifi.WifiManager",
			"STATUS_SUGGESTION_APPROVAL_PENDING"
		);
	}
	inline jint WifiManager::STATUS_SUGGESTION_APPROVAL_REJECTED_BY_USER()
	{
		return getStaticField<jint>(
			"android.net.wifi.WifiManager",
			"STATUS_SUGGESTION_APPROVAL_REJECTED_BY_USER"
		);
	}
	inline jint WifiManager::STATUS_SUGGESTION_APPROVAL_UNKNOWN()
	{
		return getStaticField<jint>(
			"android.net.wifi.WifiManager",
			"STATUS_SUGGESTION_APPROVAL_UNKNOWN"
		);
	}
	inline jint WifiManager::STATUS_SUGGESTION_CONNECTION_FAILURE_ASSOCIATION()
	{
		return getStaticField<jint>(
			"android.net.wifi.WifiManager",
			"STATUS_SUGGESTION_CONNECTION_FAILURE_ASSOCIATION"
		);
	}
	inline jint WifiManager::STATUS_SUGGESTION_CONNECTION_FAILURE_AUTHENTICATION()
	{
		return getStaticField<jint>(
			"android.net.wifi.WifiManager",
			"STATUS_SUGGESTION_CONNECTION_FAILURE_AUTHENTICATION"
		);
	}
	inline jint WifiManager::STATUS_SUGGESTION_CONNECTION_FAILURE_IP_PROVISIONING()
	{
		return getStaticField<jint>(
			"android.net.wifi.WifiManager",
			"STATUS_SUGGESTION_CONNECTION_FAILURE_IP_PROVISIONING"
		);
	}
	inline jint WifiManager::STATUS_SUGGESTION_CONNECTION_FAILURE_UNKNOWN()
	{
		return getStaticField<jint>(
			"android.net.wifi.WifiManager",
			"STATUS_SUGGESTION_CONNECTION_FAILURE_UNKNOWN"
		);
	}
	inline JString WifiManager::SUPPLICANT_CONNECTION_CHANGE_ACTION()
	{
		return getStaticObjectField(
			"android.net.wifi.WifiManager",
			"SUPPLICANT_CONNECTION_CHANGE_ACTION",
			"Ljava/lang/String;"
		);
	}
	inline JString WifiManager::SUPPLICANT_STATE_CHANGED_ACTION()
	{
		return getStaticObjectField(
			"android.net.wifi.WifiManager",
			"SUPPLICANT_STATE_CHANGED_ACTION",
			"Ljava/lang/String;"
		);
	}
	inline JString WifiManager::UNKNOWN_SSID()
	{
		return getStaticObjectField(
			"android.net.wifi.WifiManager",
			"UNKNOWN_SSID",
			"Ljava/lang/String;"
		);
	}
	inline jint WifiManager::WIFI_INTERFACE_TYPE_AP()
	{
		return getStaticField<jint>(
			"android.net.wifi.WifiManager",
			"WIFI_INTERFACE_TYPE_AP"
		);
	}
	inline jint WifiManager::WIFI_INTERFACE_TYPE_AWARE()
	{
		return getStaticField<jint>(
			"android.net.wifi.WifiManager",
			"WIFI_INTERFACE_TYPE_AWARE"
		);
	}
	inline jint WifiManager::WIFI_INTERFACE_TYPE_DIRECT()
	{
		return getStaticField<jint>(
			"android.net.wifi.WifiManager",
			"WIFI_INTERFACE_TYPE_DIRECT"
		);
	}
	inline jint WifiManager::WIFI_INTERFACE_TYPE_STA()
	{
		return getStaticField<jint>(
			"android.net.wifi.WifiManager",
			"WIFI_INTERFACE_TYPE_STA"
		);
	}
	inline jint WifiManager::WIFI_MODE_FULL()
	{
		return getStaticField<jint>(
			"android.net.wifi.WifiManager",
			"WIFI_MODE_FULL"
		);
	}
	inline jint WifiManager::WIFI_MODE_FULL_HIGH_PERF()
	{
		return getStaticField<jint>(
			"android.net.wifi.WifiManager",
			"WIFI_MODE_FULL_HIGH_PERF"
		);
	}
	inline jint WifiManager::WIFI_MODE_FULL_LOW_LATENCY()
	{
		return getStaticField<jint>(
			"android.net.wifi.WifiManager",
			"WIFI_MODE_FULL_LOW_LATENCY"
		);
	}
	inline jint WifiManager::WIFI_MODE_SCAN_ONLY()
	{
		return getStaticField<jint>(
			"android.net.wifi.WifiManager",
			"WIFI_MODE_SCAN_ONLY"
		);
	}
	inline jint WifiManager::WIFI_MULTI_INTERNET_MODE_DBS_AP()
	{
		return getStaticField<jint>(
			"android.net.wifi.WifiManager",
			"WIFI_MULTI_INTERNET_MODE_DBS_AP"
		);
	}
	inline jint WifiManager::WIFI_MULTI_INTERNET_MODE_DISABLED()
	{
		return getStaticField<jint>(
			"android.net.wifi.WifiManager",
			"WIFI_MULTI_INTERNET_MODE_DISABLED"
		);
	}
	inline jint WifiManager::WIFI_MULTI_INTERNET_MODE_MULTI_AP()
	{
		return getStaticField<jint>(
			"android.net.wifi.WifiManager",
			"WIFI_MULTI_INTERNET_MODE_MULTI_AP"
		);
	}
	inline JString WifiManager::WIFI_STATE_CHANGED_ACTION()
	{
		return getStaticObjectField(
			"android.net.wifi.WifiManager",
			"WIFI_STATE_CHANGED_ACTION",
			"Ljava/lang/String;"
		);
	}
	inline jint WifiManager::WIFI_STATE_DISABLED()
	{
		return getStaticField<jint>(
			"android.net.wifi.WifiManager",
			"WIFI_STATE_DISABLED"
		);
	}
	inline jint WifiManager::WIFI_STATE_DISABLING()
	{
		return getStaticField<jint>(
			"android.net.wifi.WifiManager",
			"WIFI_STATE_DISABLING"
		);
	}
	inline jint WifiManager::WIFI_STATE_ENABLED()
	{
		return getStaticField<jint>(
			"android.net.wifi.WifiManager",
			"WIFI_STATE_ENABLED"
		);
	}
	inline jint WifiManager::WIFI_STATE_ENABLING()
	{
		return getStaticField<jint>(
			"android.net.wifi.WifiManager",
			"WIFI_STATE_ENABLING"
		);
	}
	inline jint WifiManager::WIFI_STATE_UNKNOWN()
	{
		return getStaticField<jint>(
			"android.net.wifi.WifiManager",
			"WIFI_STATE_UNKNOWN"
		);
	}
	inline jint WifiManager::WPS_AUTH_FAILURE()
	{
		return getStaticField<jint>(
			"android.net.wifi.WifiManager",
			"WPS_AUTH_FAILURE"
		);
	}
	inline jint WifiManager::WPS_OVERLAP_ERROR()
	{
		return getStaticField<jint>(
			"android.net.wifi.WifiManager",
			"WPS_OVERLAP_ERROR"
		);
	}
	inline jint WifiManager::WPS_TIMED_OUT()
	{
		return getStaticField<jint>(
			"android.net.wifi.WifiManager",
			"WPS_TIMED_OUT"
		);
	}
	inline jint WifiManager::WPS_TKIP_ONLY_PROHIBITED()
	{
		return getStaticField<jint>(
			"android.net.wifi.WifiManager",
			"WPS_TKIP_ONLY_PROHIBITED"
		);
	}
	inline jint WifiManager::WPS_WEP_PROHIBITED()
	{
		return getStaticField<jint>(
			"android.net.wifi.WifiManager",
			"WPS_WEP_PROHIBITED"
		);
	}
	
	// Constructors
	
	// Methods
	inline jint WifiManager::calculateSignalLevel(jint arg0, jint arg1)
	{
		return callStaticMethod<jint>(
			"android.net.wifi.WifiManager",
			"calculateSignalLevel",
			"(II)I",
			arg0,
			arg1
		);
	}
	inline jint WifiManager::compareSignalLevel(jint arg0, jint arg1)
	{
		return callStaticMethod<jint>(
			"android.net.wifi.WifiManager",
			"compareSignalLevel",
			"(II)I",
			arg0,
			arg1
		);
	}
	inline void WifiManager::addLocalOnlyConnectionFailureListener(JObject arg0, JObject arg1) const
	{
		callMethod<void>(
			"addLocalOnlyConnectionFailureListener",
			"(Ljava/util/concurrent/Executor;Landroid/net/wifi/WifiManager$LocalOnlyConnectionFailureListener;)V",
			arg0.object(),
			arg1.object()
		);
	}
	inline jint WifiManager::addNetwork(android::net::wifi::WifiConfiguration arg0) const
	{
		return callMethod<jint>(
			"addNetwork",
			"(Landroid/net/wifi/WifiConfiguration;)I",
			arg0.object()
		);
	}
	inline android::net::wifi::WifiManager_AddNetworkResult WifiManager::addNetworkPrivileged(android::net::wifi::WifiConfiguration arg0) const
	{
		return callObjectMethod(
			"addNetworkPrivileged",
			"(Landroid/net/wifi/WifiConfiguration;)Landroid/net/wifi/WifiManager$AddNetworkResult;",
			arg0.object()
		);
	}
	inline jint WifiManager::addNetworkSuggestions(JObject arg0) const
	{
		return callMethod<jint>(
			"addNetworkSuggestions",
			"(Ljava/util/List;)I",
			arg0.object()
		);
	}
	inline void WifiManager::addOrUpdatePasspointConfiguration(android::net::wifi::hotspot2::PasspointConfiguration arg0) const
	{
		callMethod<void>(
			"addOrUpdatePasspointConfiguration",
			"(Landroid/net/wifi/hotspot2/PasspointConfiguration;)V",
			arg0.object()
		);
	}
	inline void WifiManager::addSuggestionConnectionStatusListener(JObject arg0, JObject arg1) const
	{
		callMethod<void>(
			"addSuggestionConnectionStatusListener",
			"(Ljava/util/concurrent/Executor;Landroid/net/wifi/WifiManager$SuggestionConnectionStatusListener;)V",
			arg0.object(),
			arg1.object()
		);
	}
	inline void WifiManager::addSuggestionUserApprovalStatusListener(JObject arg0, JObject arg1) const
	{
		callMethod<void>(
			"addSuggestionUserApprovalStatusListener",
			"(Ljava/util/concurrent/Executor;Landroid/net/wifi/WifiManager$SuggestionUserApprovalStatusListener;)V",
			arg0.object(),
			arg1.object()
		);
	}
	inline void WifiManager::addWifiStateChangedListener(JObject arg0, JObject arg1) const
	{
		callMethod<void>(
			"addWifiStateChangedListener",
			"(Ljava/util/concurrent/Executor;Landroid/net/wifi/WifiManager$WifiStateChangedListener;)V",
			arg0.object(),
			arg1.object()
		);
	}
	inline void WifiManager::allowAutojoinGlobal(jboolean arg0) const
	{
		callMethod<void>(
			"allowAutojoinGlobal",
			"(Z)V",
			arg0
		);
	}
	inline jint WifiManager::calculateSignalLevel(jint arg0) const
	{
		return callMethod<jint>(
			"calculateSignalLevel",
			"(I)I",
			arg0
		);
	}
	inline void WifiManager::cancelWps(android::net::wifi::WifiManager_WpsCallback arg0) const
	{
		callMethod<void>(
			"cancelWps",
			"(Landroid/net/wifi/WifiManager$WpsCallback;)V",
			arg0.object()
		);
	}
	inline android::net::wifi::WifiManager_MulticastLock WifiManager::createMulticastLock(JString arg0) const
	{
		return callObjectMethod(
			"createMulticastLock",
			"(Ljava/lang/String;)Landroid/net/wifi/WifiManager$MulticastLock;",
			arg0.object<jstring>()
		);
	}
	inline android::net::wifi::WifiManager_WifiLock WifiManager::createWifiLock(JString arg0) const
	{
		return callObjectMethod(
			"createWifiLock",
			"(Ljava/lang/String;)Landroid/net/wifi/WifiManager$WifiLock;",
			arg0.object<jstring>()
		);
	}
	inline android::net::wifi::WifiManager_WifiLock WifiManager::createWifiLock(jint arg0, JString arg1) const
	{
		return callObjectMethod(
			"createWifiLock",
			"(ILjava/lang/String;)Landroid/net/wifi/WifiManager$WifiLock;",
			arg0,
			arg1.object<jstring>()
		);
	}
	inline jboolean WifiManager::disableNetwork(jint arg0) const
	{
		return callMethod<jboolean>(
			"disableNetwork",
			"(I)Z",
			arg0
		);
	}
	inline void WifiManager::disallowCurrentSuggestedNetwork(android::net::wifi::BlockingOption arg0) const
	{
		callMethod<void>(
			"disallowCurrentSuggestedNetwork",
			"(Landroid/net/wifi/BlockingOption;)V",
			arg0.object()
		);
	}
	inline jboolean WifiManager::disconnect() const
	{
		return callMethod<jboolean>(
			"disconnect",
			"()Z"
		);
	}
	inline jboolean WifiManager::enableNetwork(jint arg0, jboolean arg1) const
	{
		return callMethod<jboolean>(
			"enableNetwork",
			"(IZ)Z",
			arg0,
			arg1
		);
	}
	inline void WifiManager::flushPasspointAnqpCache() const
	{
		callMethod<void>(
			"flushPasspointAnqpCache",
			"()V"
		);
	}
	inline JObject WifiManager::getAllowedChannels(jint arg0, jint arg1) const
	{
		return callObjectMethod(
			"getAllowedChannels",
			"(II)Ljava/util/List;",
			arg0,
			arg1
		);
	}
	inline JObject WifiManager::getCallerConfiguredNetworks() const
	{
		return callObjectMethod(
			"getCallerConfiguredNetworks",
			"()Ljava/util/List;"
		);
	}
	inline void WifiManager::getChannelData(JObject arg0, JObject arg1) const
	{
		callMethod<void>(
			"getChannelData",
			"(Ljava/util/concurrent/Executor;Ljava/util/function/Consumer;)V",
			arg0.object(),
			arg1.object()
		);
	}
	inline JObject WifiManager::getConfiguredNetworks() const
	{
		return callObjectMethod(
			"getConfiguredNetworks",
			"()Ljava/util/List;"
		);
	}
	inline android::net::wifi::WifiInfo WifiManager::getConnectionInfo() const
	{
		return callObjectMethod(
			"getConnectionInfo",
			"()Landroid/net/wifi/WifiInfo;"
		);
	}
	inline android::net::DhcpInfo WifiManager::getDhcpInfo() const
	{
		return callObjectMethod(
			"getDhcpInfo",
			"()Landroid/net/DhcpInfo;"
		);
	}
	inline jint WifiManager::getMaxNumberOfChannelsPerNetworkSpecifierRequest() const
	{
		return callMethod<jint>(
			"getMaxNumberOfChannelsPerNetworkSpecifierRequest",
			"()I"
		);
	}
	inline jint WifiManager::getMaxNumberOfNetworkSuggestionsPerApp() const
	{
		return callMethod<jint>(
			"getMaxNumberOfNetworkSuggestionsPerApp",
			"()I"
		);
	}
	inline jint WifiManager::getMaxSignalLevel() const
	{
		return callMethod<jint>(
			"getMaxSignalLevel",
			"()I"
		);
	}
	inline void WifiManager::getMaxSupportedConcurrentTdlsSessions(JObject arg0, JObject arg1) const
	{
		callMethod<void>(
			"getMaxSupportedConcurrentTdlsSessions",
			"(Ljava/util/concurrent/Executor;Ljava/util/function/Consumer;)V",
			arg0.object(),
			arg1.object()
		);
	}
	inline JObject WifiManager::getNetworkSuggestions() const
	{
		return callObjectMethod(
			"getNetworkSuggestions",
			"()Ljava/util/List;"
		);
	}
	inline void WifiManager::getNumberOfEnabledTdlsSessions(JObject arg0, JObject arg1) const
	{
		callMethod<void>(
			"getNumberOfEnabledTdlsSessions",
			"(Ljava/util/concurrent/Executor;Ljava/util/function/Consumer;)V",
			arg0.object(),
			arg1.object()
		);
	}
	inline JObject WifiManager::getPasspointConfigurations() const
	{
		return callObjectMethod(
			"getPasspointConfigurations",
			"()Ljava/util/List;"
		);
	}
	inline void WifiManager::getPerSsidRoamingModes(JObject arg0, JObject arg1) const
	{
		callMethod<void>(
			"getPerSsidRoamingModes",
			"(Ljava/util/concurrent/Executor;Ljava/util/function/Consumer;)V",
			arg0.object(),
			arg1.object()
		);
	}
	inline JObject WifiManager::getScanResults() const
	{
		return callObjectMethod(
			"getScanResults",
			"()Ljava/util/List;"
		);
	}
	inline jint WifiManager::getStaConcurrencyForMultiInternetMode() const
	{
		return callMethod<jint>(
			"getStaConcurrencyForMultiInternetMode",
			"()I"
		);
	}
	inline JObject WifiManager::getUsableChannels(jint arg0, jint arg1) const
	{
		return callObjectMethod(
			"getUsableChannels",
			"(II)Ljava/util/List;",
			arg0,
			arg1
		);
	}
	inline jint WifiManager::getWifiState() const
	{
		return callMethod<jint>(
			"getWifiState",
			"()I"
		);
	}
	inline jboolean WifiManager::is24GHzBandSupported() const
	{
		return callMethod<jboolean>(
			"is24GHzBandSupported",
			"()Z"
		);
	}
	inline jboolean WifiManager::is5GHzBandSupported() const
	{
		return callMethod<jboolean>(
			"is5GHzBandSupported",
			"()Z"
		);
	}
	inline jboolean WifiManager::is60GHzBandSupported() const
	{
		return callMethod<jboolean>(
			"is60GHzBandSupported",
			"()Z"
		);
	}
	inline jboolean WifiManager::is6GHzBandSupported() const
	{
		return callMethod<jboolean>(
			"is6GHzBandSupported",
			"()Z"
		);
	}
	inline jboolean WifiManager::isAggressiveRoamingModeSupported() const
	{
		return callMethod<jboolean>(
			"isAggressiveRoamingModeSupported",
			"()Z"
		);
	}
	inline jboolean WifiManager::isAutoWakeupEnabled() const
	{
		return callMethod<jboolean>(
			"isAutoWakeupEnabled",
			"()Z"
		);
	}
	inline jboolean WifiManager::isBridgedApConcurrencySupported() const
	{
		return callMethod<jboolean>(
			"isBridgedApConcurrencySupported",
			"()Z"
		);
	}
	inline jboolean WifiManager::isCarrierNetworkOffloadEnabled(jint arg0, jboolean arg1) const
	{
		return callMethod<jboolean>(
			"isCarrierNetworkOffloadEnabled",
			"(IZ)Z",
			arg0,
			arg1
		);
	}
	inline jboolean WifiManager::isD2dSupportedWhenInfraStaDisabled() const
	{
		return callMethod<jboolean>(
			"isD2dSupportedWhenInfraStaDisabled",
			"()Z"
		);
	}
	inline jboolean WifiManager::isDecoratedIdentitySupported() const
	{
		return callMethod<jboolean>(
			"isDecoratedIdentitySupported",
			"()Z"
		);
	}
	inline jboolean WifiManager::isDeviceToApRttSupported() const
	{
		return callMethod<jboolean>(
			"isDeviceToApRttSupported",
			"()Z"
		);
	}
	inline jboolean WifiManager::isDualBandSimultaneousSupported() const
	{
		return callMethod<jboolean>(
			"isDualBandSimultaneousSupported",
			"()Z"
		);
	}
	inline jboolean WifiManager::isEasyConnectDppAkmSupported() const
	{
		return callMethod<jboolean>(
			"isEasyConnectDppAkmSupported",
			"()Z"
		);
	}
	inline jboolean WifiManager::isEasyConnectEnrolleeResponderModeSupported() const
	{
		return callMethod<jboolean>(
			"isEasyConnectEnrolleeResponderModeSupported",
			"()Z"
		);
	}
	inline jboolean WifiManager::isEasyConnectSupported() const
	{
		return callMethod<jboolean>(
			"isEasyConnectSupported",
			"()Z"
		);
	}
	inline jboolean WifiManager::isEnhancedOpenSupported() const
	{
		return callMethod<jboolean>(
			"isEnhancedOpenSupported",
			"()Z"
		);
	}
	inline jboolean WifiManager::isEnhancedPowerReportingSupported() const
	{
		return callMethod<jboolean>(
			"isEnhancedPowerReportingSupported",
			"()Z"
		);
	}
	inline jboolean WifiManager::isMakeBeforeBreakWifiSwitchingSupported() const
	{
		return callMethod<jboolean>(
			"isMakeBeforeBreakWifiSwitchingSupported",
			"()Z"
		);
	}
	inline jboolean WifiManager::isP2pSupported() const
	{
		return callMethod<jboolean>(
			"isP2pSupported",
			"()Z"
		);
	}
	inline jboolean WifiManager::isPasspointTermsAndConditionsSupported() const
	{
		return callMethod<jboolean>(
			"isPasspointTermsAndConditionsSupported",
			"()Z"
		);
	}
	inline jboolean WifiManager::isPreferredNetworkOffloadSupported() const
	{
		return callMethod<jboolean>(
			"isPreferredNetworkOffloadSupported",
			"()Z"
		);
	}
	inline jboolean WifiManager::isScanAlwaysAvailable() const
	{
		return callMethod<jboolean>(
			"isScanAlwaysAvailable",
			"()Z"
		);
	}
	inline jboolean WifiManager::isScanThrottleEnabled() const
	{
		return callMethod<jboolean>(
			"isScanThrottleEnabled",
			"()Z"
		);
	}
	inline jboolean WifiManager::isStaApConcurrencySupported() const
	{
		return callMethod<jboolean>(
			"isStaApConcurrencySupported",
			"()Z"
		);
	}
	inline jboolean WifiManager::isStaBridgedApConcurrencySupported() const
	{
		return callMethod<jboolean>(
			"isStaBridgedApConcurrencySupported",
			"()Z"
		);
	}
	inline jboolean WifiManager::isStaConcurrencyForLocalOnlyConnectionsSupported() const
	{
		return callMethod<jboolean>(
			"isStaConcurrencyForLocalOnlyConnectionsSupported",
			"()Z"
		);
	}
	inline jboolean WifiManager::isStaConcurrencyForMultiInternetSupported() const
	{
		return callMethod<jboolean>(
			"isStaConcurrencyForMultiInternetSupported",
			"()Z"
		);
	}
	inline void WifiManager::isTdlsOperationCurrentlyAvailable(JObject arg0, JObject arg1) const
	{
		callMethod<void>(
			"isTdlsOperationCurrentlyAvailable",
			"(Ljava/util/concurrent/Executor;Ljava/util/function/Consumer;)V",
			arg0.object(),
			arg1.object()
		);
	}
	inline jboolean WifiManager::isTdlsSupported() const
	{
		return callMethod<jboolean>(
			"isTdlsSupported",
			"()Z"
		);
	}
	inline jboolean WifiManager::isTidToLinkMappingNegotiationSupported() const
	{
		return callMethod<jboolean>(
			"isTidToLinkMappingNegotiationSupported",
			"()Z"
		);
	}
	inline jboolean WifiManager::isTlsMinimumVersionSupported() const
	{
		return callMethod<jboolean>(
			"isTlsMinimumVersionSupported",
			"()Z"
		);
	}
	inline jboolean WifiManager::isTlsV13Supported() const
	{
		return callMethod<jboolean>(
			"isTlsV13Supported",
			"()Z"
		);
	}
	inline jboolean WifiManager::isTrustOnFirstUseSupported() const
	{
		return callMethod<jboolean>(
			"isTrustOnFirstUseSupported",
			"()Z"
		);
	}
	inline jboolean WifiManager::isWapiSupported() const
	{
		return callMethod<jboolean>(
			"isWapiSupported",
			"()Z"
		);
	}
	inline jboolean WifiManager::isWepSupported() const
	{
		return callMethod<jboolean>(
			"isWepSupported",
			"()Z"
		);
	}
	inline jboolean WifiManager::isWifiDisplayR2Supported() const
	{
		return callMethod<jboolean>(
			"isWifiDisplayR2Supported",
			"()Z"
		);
	}
	inline jboolean WifiManager::isWifiEnabled() const
	{
		return callMethod<jboolean>(
			"isWifiEnabled",
			"()Z"
		);
	}
	inline jboolean WifiManager::isWifiPasspointEnabled() const
	{
		return callMethod<jboolean>(
			"isWifiPasspointEnabled",
			"()Z"
		);
	}
	inline jboolean WifiManager::isWifiStandardSupported(jint arg0) const
	{
		return callMethod<jboolean>(
			"isWifiStandardSupported",
			"(I)Z",
			arg0
		);
	}
	inline jboolean WifiManager::isWpa3SaeH2eSupported() const
	{
		return callMethod<jboolean>(
			"isWpa3SaeH2eSupported",
			"()Z"
		);
	}
	inline jboolean WifiManager::isWpa3SaePublicKeySupported() const
	{
		return callMethod<jboolean>(
			"isWpa3SaePublicKeySupported",
			"()Z"
		);
	}
	inline jboolean WifiManager::isWpa3SaeSupported() const
	{
		return callMethod<jboolean>(
			"isWpa3SaeSupported",
			"()Z"
		);
	}
	inline jboolean WifiManager::isWpa3SuiteBSupported() const
	{
		return callMethod<jboolean>(
			"isWpa3SuiteBSupported",
			"()Z"
		);
	}
	inline jboolean WifiManager::isWpaPersonalSupported() const
	{
		return callMethod<jboolean>(
			"isWpaPersonalSupported",
			"()Z"
		);
	}
	inline jboolean WifiManager::pingSupplicant() const
	{
		return callMethod<jboolean>(
			"pingSupplicant",
			"()Z"
		);
	}
	inline void WifiManager::queryAutojoinGlobal(JObject arg0, JObject arg1) const
	{
		callMethod<void>(
			"queryAutojoinGlobal",
			"(Ljava/util/concurrent/Executor;Ljava/util/function/Consumer;)V",
			arg0.object(),
			arg1.object()
		);
	}
	inline void WifiManager::querySendDhcpHostnameRestriction(JObject arg0, JObject arg1) const
	{
		callMethod<void>(
			"querySendDhcpHostnameRestriction",
			"(Ljava/util/concurrent/Executor;Ljava/util/function/IntConsumer;)V",
			arg0.object(),
			arg1.object()
		);
	}
	inline jboolean WifiManager::reassociate() const
	{
		return callMethod<jboolean>(
			"reassociate",
			"()Z"
		);
	}
	inline jboolean WifiManager::reconnect() const
	{
		return callMethod<jboolean>(
			"reconnect",
			"()Z"
		);
	}
	inline void WifiManager::registerScanResultsCallback(JObject arg0, android::net::wifi::WifiManager_ScanResultsCallback arg1) const
	{
		callMethod<void>(
			"registerScanResultsCallback",
			"(Ljava/util/concurrent/Executor;Landroid/net/wifi/WifiManager$ScanResultsCallback;)V",
			arg0.object(),
			arg1.object()
		);
	}
	inline void WifiManager::registerSubsystemRestartTrackingCallback(JObject arg0, android::net::wifi::WifiManager_SubsystemRestartTrackingCallback arg1) const
	{
		callMethod<void>(
			"registerSubsystemRestartTrackingCallback",
			"(Ljava/util/concurrent/Executor;Landroid/net/wifi/WifiManager$SubsystemRestartTrackingCallback;)V",
			arg0.object(),
			arg1.object()
		);
	}
	inline void WifiManager::removeLocalOnlyConnectionFailureListener(JObject arg0) const
	{
		callMethod<void>(
			"removeLocalOnlyConnectionFailureListener",
			"(Landroid/net/wifi/WifiManager$LocalOnlyConnectionFailureListener;)V",
			arg0.object()
		);
	}
	inline jboolean WifiManager::removeNetwork(jint arg0) const
	{
		return callMethod<jboolean>(
			"removeNetwork",
			"(I)Z",
			arg0
		);
	}
	inline jint WifiManager::removeNetworkSuggestions(JObject arg0) const
	{
		return callMethod<jint>(
			"removeNetworkSuggestions",
			"(Ljava/util/List;)I",
			arg0.object()
		);
	}
	inline jint WifiManager::removeNetworkSuggestions(JObject arg0, jint arg1) const
	{
		return callMethod<jint>(
			"removeNetworkSuggestions",
			"(Ljava/util/List;I)I",
			arg0.object(),
			arg1
		);
	}
	inline jboolean WifiManager::removeNonCallerConfiguredNetworks() const
	{
		return callMethod<jboolean>(
			"removeNonCallerConfiguredNetworks",
			"()Z"
		);
	}
	inline void WifiManager::removePasspointConfiguration(JString arg0) const
	{
		callMethod<void>(
			"removePasspointConfiguration",
			"(Ljava/lang/String;)V",
			arg0.object<jstring>()
		);
	}
	inline void WifiManager::removePerSsidRoamingMode(android::net::wifi::WifiSsid arg0) const
	{
		callMethod<void>(
			"removePerSsidRoamingMode",
			"(Landroid/net/wifi/WifiSsid;)V",
			arg0.object()
		);
	}
	inline void WifiManager::removeSuggestionConnectionStatusListener(JObject arg0) const
	{
		callMethod<void>(
			"removeSuggestionConnectionStatusListener",
			"(Landroid/net/wifi/WifiManager$SuggestionConnectionStatusListener;)V",
			arg0.object()
		);
	}
	inline void WifiManager::removeSuggestionUserApprovalStatusListener(JObject arg0) const
	{
		callMethod<void>(
			"removeSuggestionUserApprovalStatusListener",
			"(Landroid/net/wifi/WifiManager$SuggestionUserApprovalStatusListener;)V",
			arg0.object()
		);
	}
	inline void WifiManager::removeWifiStateChangedListener(JObject arg0) const
	{
		callMethod<void>(
			"removeWifiStateChangedListener",
			"(Landroid/net/wifi/WifiManager$WifiStateChangedListener;)V",
			arg0.object()
		);
	}
	inline void WifiManager::reportCreateInterfaceImpact(jint arg0, jboolean arg1, JObject arg2, JObject arg3) const
	{
		callMethod<void>(
			"reportCreateInterfaceImpact",
			"(IZLjava/util/concurrent/Executor;Ljava/util/function/BiConsumer;)V",
			arg0,
			arg1,
			arg2.object(),
			arg3.object()
		);
	}
	inline jboolean WifiManager::saveConfiguration() const
	{
		return callMethod<jboolean>(
			"saveConfiguration",
			"()Z"
		);
	}
	inline void WifiManager::setPerSsidRoamingMode(android::net::wifi::WifiSsid arg0, jint arg1) const
	{
		callMethod<void>(
			"setPerSsidRoamingMode",
			"(Landroid/net/wifi/WifiSsid;I)V",
			arg0.object(),
			arg1
		);
	}
	inline void WifiManager::setSendDhcpHostnameRestriction(jint arg0) const
	{
		callMethod<void>(
			"setSendDhcpHostnameRestriction",
			"(I)V",
			arg0
		);
	}
	inline void WifiManager::setTdlsEnabled(java::net::InetAddress arg0, jboolean arg1) const
	{
		callMethod<void>(
			"setTdlsEnabled",
			"(Ljava/net/InetAddress;Z)V",
			arg0.object(),
			arg1
		);
	}
	inline void WifiManager::setTdlsEnabled(java::net::InetAddress arg0, jboolean arg1, JObject arg2, JObject arg3) const
	{
		callMethod<void>(
			"setTdlsEnabled",
			"(Ljava/net/InetAddress;ZLjava/util/concurrent/Executor;Ljava/util/function/Consumer;)V",
			arg0.object(),
			arg1,
			arg2.object(),
			arg3.object()
		);
	}
	inline void WifiManager::setTdlsEnabledWithMacAddress(JString arg0, jboolean arg1) const
	{
		callMethod<void>(
			"setTdlsEnabledWithMacAddress",
			"(Ljava/lang/String;Z)V",
			arg0.object<jstring>(),
			arg1
		);
	}
	inline void WifiManager::setTdlsEnabledWithMacAddress(JString arg0, jboolean arg1, JObject arg2, JObject arg3) const
	{
		callMethod<void>(
			"setTdlsEnabledWithMacAddress",
			"(Ljava/lang/String;ZLjava/util/concurrent/Executor;Ljava/util/function/Consumer;)V",
			arg0.object<jstring>(),
			arg1,
			arg2.object(),
			arg3.object()
		);
	}
	inline jboolean WifiManager::setWifiEnabled(jboolean arg0) const
	{
		return callMethod<jboolean>(
			"setWifiEnabled",
			"(Z)Z",
			arg0
		);
	}
	inline void WifiManager::startLocalOnlyHotspot(android::net::wifi::WifiManager_LocalOnlyHotspotCallback arg0, android::os::Handler arg1) const
	{
		callMethod<void>(
			"startLocalOnlyHotspot",
			"(Landroid/net/wifi/WifiManager$LocalOnlyHotspotCallback;Landroid/os/Handler;)V",
			arg0.object(),
			arg1.object()
		);
	}
	inline void WifiManager::startLocalOnlyHotspotWithConfiguration(android::net::wifi::SoftApConfiguration arg0, JObject arg1, android::net::wifi::WifiManager_LocalOnlyHotspotCallback arg2) const
	{
		callMethod<void>(
			"startLocalOnlyHotspotWithConfiguration",
			"(Landroid/net/wifi/SoftApConfiguration;Ljava/util/concurrent/Executor;Landroid/net/wifi/WifiManager$LocalOnlyHotspotCallback;)V",
			arg0.object(),
			arg1.object(),
			arg2.object()
		);
	}
	inline jboolean WifiManager::startScan() const
	{
		return callMethod<jboolean>(
			"startScan",
			"()Z"
		);
	}
	inline void WifiManager::startWps(android::net::wifi::WpsInfo arg0, android::net::wifi::WifiManager_WpsCallback arg1) const
	{
		callMethod<void>(
			"startWps",
			"(Landroid/net/wifi/WpsInfo;Landroid/net/wifi/WifiManager$WpsCallback;)V",
			arg0.object(),
			arg1.object()
		);
	}
	inline void WifiManager::unregisterScanResultsCallback(android::net::wifi::WifiManager_ScanResultsCallback arg0) const
	{
		callMethod<void>(
			"unregisterScanResultsCallback",
			"(Landroid/net/wifi/WifiManager$ScanResultsCallback;)V",
			arg0.object()
		);
	}
	inline void WifiManager::unregisterSubsystemRestartTrackingCallback(android::net::wifi::WifiManager_SubsystemRestartTrackingCallback arg0) const
	{
		callMethod<void>(
			"unregisterSubsystemRestartTrackingCallback",
			"(Landroid/net/wifi/WifiManager$SubsystemRestartTrackingCallback;)V",
			arg0.object()
		);
	}
	inline jint WifiManager::updateNetwork(android::net::wifi::WifiConfiguration arg0) const
	{
		return callMethod<jint>(
			"updateNetwork",
			"(Landroid/net/wifi/WifiConfiguration;)I",
			arg0.object()
		);
	}
	inline jboolean WifiManager::validateSoftApConfiguration(android::net::wifi::SoftApConfiguration arg0) const
	{
		return callMethod<jboolean>(
			"validateSoftApConfiguration",
			"(Landroid/net/wifi/SoftApConfiguration;)Z",
			arg0.object()
		);
	}
} // namespace android::net::wifi

// Base class headers

#ifdef QT_ANDROID_API_AUTOUSE
using namespace android::net::wifi;
#endif
