#pragma once

#include "../../JFloatArray.hpp"
#include "../../JIntArray.hpp"
#include "../graphics/Canvas.def.hpp"
#include "../graphics/Paint.def.hpp"
#include "../graphics/Paint_FontMetrics.def.hpp"
#include "../graphics/Path.def.hpp"
#include "../graphics/Rect.def.hpp"
#include "../graphics/RectF.def.hpp"
#include "../graphics/text/LineBreakConfig.def.hpp"
#include "./Layout_Alignment.def.hpp"
#include "./Layout_Directions.def.hpp"
#include "./SegmentFinder.def.hpp"
#include "./TextPaint.def.hpp"
#include "./TextUtils_TruncateAt.def.hpp"
#include "../../JString.hpp"
#include "./Layout.def.hpp"

namespace android::text
{
	// Fields
	inline jint Layout::BREAK_STRATEGY_BALANCED()
	{
		return getStaticField<jint>(
			"android.text.Layout",
			"BREAK_STRATEGY_BALANCED"
		);
	}
	inline jint Layout::BREAK_STRATEGY_HIGH_QUALITY()
	{
		return getStaticField<jint>(
			"android.text.Layout",
			"BREAK_STRATEGY_HIGH_QUALITY"
		);
	}
	inline jint Layout::BREAK_STRATEGY_SIMPLE()
	{
		return getStaticField<jint>(
			"android.text.Layout",
			"BREAK_STRATEGY_SIMPLE"
		);
	}
	inline jfloat Layout::DEFAULT_LINESPACING_ADDITION()
	{
		return getStaticField<jfloat>(
			"android.text.Layout",
			"DEFAULT_LINESPACING_ADDITION"
		);
	}
	inline jfloat Layout::DEFAULT_LINESPACING_MULTIPLIER()
	{
		return getStaticField<jfloat>(
			"android.text.Layout",
			"DEFAULT_LINESPACING_MULTIPLIER"
		);
	}
	inline jint Layout::DIR_LEFT_TO_RIGHT()
	{
		return getStaticField<jint>(
			"android.text.Layout",
			"DIR_LEFT_TO_RIGHT"
		);
	}
	inline jint Layout::DIR_RIGHT_TO_LEFT()
	{
		return getStaticField<jint>(
			"android.text.Layout",
			"DIR_RIGHT_TO_LEFT"
		);
	}
	inline jint Layout::HYPHENATION_FREQUENCY_FULL()
	{
		return getStaticField<jint>(
			"android.text.Layout",
			"HYPHENATION_FREQUENCY_FULL"
		);
	}
	inline jint Layout::HYPHENATION_FREQUENCY_FULL_FAST()
	{
		return getStaticField<jint>(
			"android.text.Layout",
			"HYPHENATION_FREQUENCY_FULL_FAST"
		);
	}
	inline jint Layout::HYPHENATION_FREQUENCY_NONE()
	{
		return getStaticField<jint>(
			"android.text.Layout",
			"HYPHENATION_FREQUENCY_NONE"
		);
	}
	inline jint Layout::HYPHENATION_FREQUENCY_NORMAL()
	{
		return getStaticField<jint>(
			"android.text.Layout",
			"HYPHENATION_FREQUENCY_NORMAL"
		);
	}
	inline jint Layout::HYPHENATION_FREQUENCY_NORMAL_FAST()
	{
		return getStaticField<jint>(
			"android.text.Layout",
			"HYPHENATION_FREQUENCY_NORMAL_FAST"
		);
	}
	inline JObject Layout::INCLUSION_STRATEGY_ANY_OVERLAP()
	{
		return getStaticObjectField(
			"android.text.Layout",
			"INCLUSION_STRATEGY_ANY_OVERLAP",
			"Landroid/text/Layout$TextInclusionStrategy;"
		);
	}
	inline JObject Layout::INCLUSION_STRATEGY_CONTAINS_ALL()
	{
		return getStaticObjectField(
			"android.text.Layout",
			"INCLUSION_STRATEGY_CONTAINS_ALL",
			"Landroid/text/Layout$TextInclusionStrategy;"
		);
	}
	inline JObject Layout::INCLUSION_STRATEGY_CONTAINS_CENTER()
	{
		return getStaticObjectField(
			"android.text.Layout",
			"INCLUSION_STRATEGY_CONTAINS_CENTER",
			"Landroid/text/Layout$TextInclusionStrategy;"
		);
	}
	inline jint Layout::JUSTIFICATION_MODE_INTER_CHARACTER()
	{
		return getStaticField<jint>(
			"android.text.Layout",
			"JUSTIFICATION_MODE_INTER_CHARACTER"
		);
	}
	inline jint Layout::JUSTIFICATION_MODE_INTER_WORD()
	{
		return getStaticField<jint>(
			"android.text.Layout",
			"JUSTIFICATION_MODE_INTER_WORD"
		);
	}
	inline jint Layout::JUSTIFICATION_MODE_NONE()
	{
		return getStaticField<jint>(
			"android.text.Layout",
			"JUSTIFICATION_MODE_NONE"
		);
	}
	
	// Constructors
	
	// Methods
	inline jfloat Layout::getDesiredWidth(JString arg0, android::text::TextPaint arg1)
	{
		return callStaticMethod<jfloat>(
			"android.text.Layout",
			"getDesiredWidth",
			"(Ljava/lang/CharSequence;Landroid/text/TextPaint;)F",
			arg0.object<jstring>(),
			arg1.object()
		);
	}
	inline jfloat Layout::getDesiredWidth(JString arg0, jint arg1, jint arg2, android::text::TextPaint arg3)
	{
		return callStaticMethod<jfloat>(
			"android.text.Layout",
			"getDesiredWidth",
			"(Ljava/lang/CharSequence;IILandroid/text/TextPaint;)F",
			arg0.object<jstring>(),
			arg1,
			arg2,
			arg3.object()
		);
	}
	inline android::graphics::RectF Layout::computeDrawingBoundingBox() const
	{
		return callObjectMethod(
			"computeDrawingBoundingBox",
			"()Landroid/graphics/RectF;"
		);
	}
	inline void Layout::draw(android::graphics::Canvas arg0) const
	{
		callMethod<void>(
			"draw",
			"(Landroid/graphics/Canvas;)V",
			arg0.object()
		);
	}
	inline void Layout::draw(android::graphics::Canvas arg0, android::graphics::Path arg1, android::graphics::Paint arg2, jint arg3) const
	{
		callMethod<void>(
			"draw",
			"(Landroid/graphics/Canvas;Landroid/graphics/Path;Landroid/graphics/Paint;I)V",
			arg0.object(),
			arg1.object(),
			arg2.object(),
			arg3
		);
	}
	inline void Layout::draw(android::graphics::Canvas arg0, JObject arg1, JObject arg2, android::graphics::Path arg3, android::graphics::Paint arg4, jint arg5) const
	{
		callMethod<void>(
			"draw",
			"(Landroid/graphics/Canvas;Ljava/util/List;Ljava/util/List;Landroid/graphics/Path;Landroid/graphics/Paint;I)V",
			arg0.object(),
			arg1.object(),
			arg2.object(),
			arg3.object(),
			arg4.object(),
			arg5
		);
	}
	inline void Layout::drawBackground(android::graphics::Canvas arg0) const
	{
		callMethod<void>(
			"drawBackground",
			"(Landroid/graphics/Canvas;)V",
			arg0.object()
		);
	}
	inline void Layout::drawText(android::graphics::Canvas arg0) const
	{
		callMethod<void>(
			"drawText",
			"(Landroid/graphics/Canvas;)V",
			arg0.object()
		);
	}
	inline void Layout::fillCharacterBounds(jint arg0, jint arg1, JFloatArray arg2, jint arg3) const
	{
		callMethod<void>(
			"fillCharacterBounds",
			"(II[FI)V",
			arg0,
			arg1,
			arg2.object<jfloatArray>(),
			arg3
		);
	}
	inline android::text::Layout_Alignment Layout::getAlignment() const
	{
		return callObjectMethod(
			"getAlignment",
			"()Landroid/text/Layout$Alignment;"
		);
	}
	inline jint Layout::getBottomPadding() const
	{
		return callMethod<jint>(
			"getBottomPadding",
			"()I"
		);
	}
	inline jint Layout::getBreakStrategy() const
	{
		return callMethod<jint>(
			"getBreakStrategy",
			"()I"
		);
	}
	inline void Layout::getCursorPath(jint arg0, android::graphics::Path arg1, JString arg2) const
	{
		callMethod<void>(
			"getCursorPath",
			"(ILandroid/graphics/Path;Ljava/lang/CharSequence;)V",
			arg0,
			arg1.object(),
			arg2.object<jstring>()
		);
	}
	inline jint Layout::getEllipsisCount(jint arg0) const
	{
		return callMethod<jint>(
			"getEllipsisCount",
			"(I)I",
			arg0
		);
	}
	inline jint Layout::getEllipsisStart(jint arg0) const
	{
		return callMethod<jint>(
			"getEllipsisStart",
			"(I)I",
			arg0
		);
	}
	inline android::text::TextUtils_TruncateAt Layout::getEllipsize() const
	{
		return callObjectMethod(
			"getEllipsize",
			"()Landroid/text/TextUtils$TruncateAt;"
		);
	}
	inline jint Layout::getEllipsizedWidth() const
	{
		return callMethod<jint>(
			"getEllipsizedWidth",
			"()I"
		);
	}
	inline jint Layout::getHeight() const
	{
		return callMethod<jint>(
			"getHeight",
			"()I"
		);
	}
	inline jint Layout::getHyphenationFrequency() const
	{
		return callMethod<jint>(
			"getHyphenationFrequency",
			"()I"
		);
	}
	inline jint Layout::getJustificationMode() const
	{
		return callMethod<jint>(
			"getJustificationMode",
			"()I"
		);
	}
	inline JIntArray Layout::getLeftIndents() const
	{
		return callObjectMethod(
			"getLeftIndents",
			"()[I"
		);
	}
	inline jint Layout::getLineAscent(jint arg0) const
	{
		return callMethod<jint>(
			"getLineAscent",
			"(I)I",
			arg0
		);
	}
	inline jint Layout::getLineBaseline(jint arg0) const
	{
		return callMethod<jint>(
			"getLineBaseline",
			"(I)I",
			arg0
		);
	}
	inline jint Layout::getLineBottom(jint arg0) const
	{
		return callMethod<jint>(
			"getLineBottom",
			"(I)I",
			arg0
		);
	}
	inline jint Layout::getLineBottom(jint arg0, jboolean arg1) const
	{
		return callMethod<jint>(
			"getLineBottom",
			"(IZ)I",
			arg0,
			arg1
		);
	}
	inline jint Layout::getLineBounds(jint arg0, android::graphics::Rect arg1) const
	{
		return callMethod<jint>(
			"getLineBounds",
			"(ILandroid/graphics/Rect;)I",
			arg0,
			arg1.object()
		);
	}
	inline android::graphics::text::LineBreakConfig Layout::getLineBreakConfig() const
	{
		return callObjectMethod(
			"getLineBreakConfig",
			"()Landroid/graphics/text/LineBreakConfig;"
		);
	}
	inline jboolean Layout::getLineContainsTab(jint arg0) const
	{
		return callMethod<jboolean>(
			"getLineContainsTab",
			"(I)Z",
			arg0
		);
	}
	inline jint Layout::getLineCount() const
	{
		return callMethod<jint>(
			"getLineCount",
			"()I"
		);
	}
	inline jint Layout::getLineDescent(jint arg0) const
	{
		return callMethod<jint>(
			"getLineDescent",
			"(I)I",
			arg0
		);
	}
	inline android::text::Layout_Directions Layout::getLineDirections(jint arg0) const
	{
		return callObjectMethod(
			"getLineDirections",
			"(I)Landroid/text/Layout$Directions;",
			arg0
		);
	}
	inline jint Layout::getLineEnd(jint arg0) const
	{
		return callMethod<jint>(
			"getLineEnd",
			"(I)I",
			arg0
		);
	}
	inline jint Layout::getLineForOffset(jint arg0) const
	{
		return callMethod<jint>(
			"getLineForOffset",
			"(I)I",
			arg0
		);
	}
	inline jint Layout::getLineForVertical(jint arg0) const
	{
		return callMethod<jint>(
			"getLineForVertical",
			"(I)I",
			arg0
		);
	}
	inline jfloat Layout::getLineLeft(jint arg0) const
	{
		return callMethod<jfloat>(
			"getLineLeft",
			"(I)F",
			arg0
		);
	}
	inline jint Layout::getLineLetterSpacingUnitCount(jint arg0, jboolean arg1) const
	{
		return callMethod<jint>(
			"getLineLetterSpacingUnitCount",
			"(IZ)I",
			arg0,
			arg1
		);
	}
	inline jfloat Layout::getLineMax(jint arg0) const
	{
		return callMethod<jfloat>(
			"getLineMax",
			"(I)F",
			arg0
		);
	}
	inline jfloat Layout::getLineRight(jint arg0) const
	{
		return callMethod<jfloat>(
			"getLineRight",
			"(I)F",
			arg0
		);
	}
	inline jfloat Layout::getLineSpacingAmount() const
	{
		return callMethod<jfloat>(
			"getLineSpacingAmount",
			"()F"
		);
	}
	inline jfloat Layout::getLineSpacingMultiplier() const
	{
		return callMethod<jfloat>(
			"getLineSpacingMultiplier",
			"()F"
		);
	}
	inline jint Layout::getLineStart(jint arg0) const
	{
		return callMethod<jint>(
			"getLineStart",
			"(I)I",
			arg0
		);
	}
	inline jint Layout::getLineTop(jint arg0) const
	{
		return callMethod<jint>(
			"getLineTop",
			"(I)I",
			arg0
		);
	}
	inline jint Layout::getLineVisibleEnd(jint arg0) const
	{
		return callMethod<jint>(
			"getLineVisibleEnd",
			"(I)I",
			arg0
		);
	}
	inline jfloat Layout::getLineWidth(jint arg0) const
	{
		return callMethod<jfloat>(
			"getLineWidth",
			"(I)F",
			arg0
		);
	}
	inline jint Layout::getMaxLines() const
	{
		return callMethod<jint>(
			"getMaxLines",
			"()I"
		);
	}
	inline android::graphics::Paint_FontMetrics Layout::getMinimumFontMetrics() const
	{
		return callObjectMethod(
			"getMinimumFontMetrics",
			"()Landroid/graphics/Paint$FontMetrics;"
		);
	}
	inline jint Layout::getOffsetForHorizontal(jint arg0, jfloat arg1) const
	{
		return callMethod<jint>(
			"getOffsetForHorizontal",
			"(IF)I",
			arg0,
			arg1
		);
	}
	inline jint Layout::getOffsetToLeftOf(jint arg0) const
	{
		return callMethod<jint>(
			"getOffsetToLeftOf",
			"(I)I",
			arg0
		);
	}
	inline jint Layout::getOffsetToRightOf(jint arg0) const
	{
		return callMethod<jint>(
			"getOffsetToRightOf",
			"(I)I",
			arg0
		);
	}
	inline android::text::TextPaint Layout::getPaint() const
	{
		return callObjectMethod(
			"getPaint",
			"()Landroid/text/TextPaint;"
		);
	}
	inline android::text::Layout_Alignment Layout::getParagraphAlignment(jint arg0) const
	{
		return callObjectMethod(
			"getParagraphAlignment",
			"(I)Landroid/text/Layout$Alignment;",
			arg0
		);
	}
	inline jint Layout::getParagraphDirection(jint arg0) const
	{
		return callMethod<jint>(
			"getParagraphDirection",
			"(I)I",
			arg0
		);
	}
	inline jint Layout::getParagraphLeft(jint arg0) const
	{
		return callMethod<jint>(
			"getParagraphLeft",
			"(I)I",
			arg0
		);
	}
	inline jint Layout::getParagraphRight(jint arg0) const
	{
		return callMethod<jint>(
			"getParagraphRight",
			"(I)I",
			arg0
		);
	}
	inline jfloat Layout::getPrimaryHorizontal(jint arg0) const
	{
		return callMethod<jfloat>(
			"getPrimaryHorizontal",
			"(I)F",
			arg0
		);
	}
	inline JIntArray Layout::getRangeForRect(android::graphics::RectF arg0, android::text::SegmentFinder arg1, JObject arg2) const
	{
		return callObjectMethod(
			"getRangeForRect",
			"(Landroid/graphics/RectF;Landroid/text/SegmentFinder;Landroid/text/Layout$TextInclusionStrategy;)[I",
			arg0.object(),
			arg1.object(),
			arg2.object()
		);
	}
	inline JIntArray Layout::getRightIndents() const
	{
		return callObjectMethod(
			"getRightIndents",
			"()[I"
		);
	}
	inline jfloat Layout::getSecondaryHorizontal(jint arg0) const
	{
		return callMethod<jfloat>(
			"getSecondaryHorizontal",
			"(I)F",
			arg0
		);
	}
	inline void Layout::getSelectionPath(jint arg0, jint arg1, android::graphics::Path arg2) const
	{
		callMethod<void>(
			"getSelectionPath",
			"(IILandroid/graphics/Path;)V",
			arg0,
			arg1,
			arg2.object()
		);
	}
	inline jboolean Layout::getShiftDrawingOffsetForStartOverhang() const
	{
		return callMethod<jboolean>(
			"getShiftDrawingOffsetForStartOverhang",
			"()Z"
		);
	}
	inline jfloat Layout::getSpacingAdd() const
	{
		return callMethod<jfloat>(
			"getSpacingAdd",
			"()F"
		);
	}
	inline jfloat Layout::getSpacingMultiplier() const
	{
		return callMethod<jfloat>(
			"getSpacingMultiplier",
			"()F"
		);
	}
	inline JString Layout::getText() const
	{
		return callObjectMethod(
			"getText",
			"()Ljava/lang/CharSequence;"
		);
	}
	inline JObject Layout::getTextDirectionHeuristic() const
	{
		return callObjectMethod(
			"getTextDirectionHeuristic",
			"()Landroid/text/TextDirectionHeuristic;"
		);
	}
	inline jint Layout::getTopPadding() const
	{
		return callMethod<jint>(
			"getTopPadding",
			"()I"
		);
	}
	inline jboolean Layout::getUseBoundsForWidth() const
	{
		return callMethod<jboolean>(
			"getUseBoundsForWidth",
			"()Z"
		);
	}
	inline jint Layout::getWidth() const
	{
		return callMethod<jint>(
			"getWidth",
			"()I"
		);
	}
	inline void Layout::increaseWidthTo(jint arg0) const
	{
		callMethod<void>(
			"increaseWidthTo",
			"(I)V",
			arg0
		);
	}
	inline jboolean Layout::isFallbackLineSpacingEnabled() const
	{
		return callMethod<jboolean>(
			"isFallbackLineSpacingEnabled",
			"()Z"
		);
	}
	inline jboolean Layout::isFontPaddingIncluded() const
	{
		return callMethod<jboolean>(
			"isFontPaddingIncluded",
			"()Z"
		);
	}
	inline jboolean Layout::isRtlCharAt(jint arg0) const
	{
		return callMethod<jboolean>(
			"isRtlCharAt",
			"(I)Z",
			arg0
		);
	}
} // namespace android::text

// Base class headers

#ifdef QT_ANDROID_API_AUTOUSE
using namespace android::text;
#endif
